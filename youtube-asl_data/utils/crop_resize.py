...
def get_clip(input_video_dir, output_video_dir, bbox_fn, rank, nshard, target_size=224, ffmpeg=None):
    os.makedirs(output_video_dir, exist_ok=True)
    vid2bbox = json.load(open(bbox_fn))
  
    for vid, bbox in tqdm(vid2bbox.items()):
        input_video_clip, output_video = os.path.join(input_video_dir, vid+'.mp4'), os.path.join(output_video_dir, vid+'.mp4')
...
def main():
    ...
    get_clip(args.raw, args.output, args.bbox, 0, 1, target_size=args.target_size, ffmpeg=args.ffmpeg)
    return
